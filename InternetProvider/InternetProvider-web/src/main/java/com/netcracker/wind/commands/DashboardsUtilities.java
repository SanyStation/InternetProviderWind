...
public class DashboardsUtilities {
    ...
    private static final Logger LOGGER
            = Logger.getLogger(DashboardsUtilities.class.getName());
    ...
    public static String getTaskUserStatus(int userId, String status, int from,
            int number) {
        AbstractFactoryDAO factoryDAO
                = FactoryCreator.getInstance().getFactory();
        ITaskDAO taskDAO = factoryDAO.createTaskDAO();
        List<Task> tasks
                = taskDAO.findByPerformerStatus(userId, status, from, number);
        return parseTaskJSON(tasks, taskDAO.findByPerformerStatus(userId,
                status, AbstractOracleDAO.DEFAULT_PAGE_NUMBER,
                AbstractOracleDAO.ALL_RECORDS).size());
    }
}
